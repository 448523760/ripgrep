...
use crate::sink::{
  Sink, SinkContext, SinkContextKind, SinkError, SinkFinish,
  SinkMatch,
};
...
impl<'s, M: Matcher, S: Sink> Core<'s, M, S> {
  ...
  pub fn match_by_line(
    &mut self,
    buf: &[u8],
  ) -> Result<bool, S::Error> {
    debug!(
      "match line by line input: {}",
      String::from_utf8_lossy(buf)
    );

    if self.is_line_by_line_fast() {
      debug!("fast line by line");
  ...
  pub fn after_context_by_line(
    ...
    if self.after_context_left == 0 {
      // 如果剩余after_context == 0 就返回：没有after_context要处理
      return Ok(true);
    }

    // context: 上次访问位置到找到的match的开始位置：这个范围表示在buf中需要进行匹配的行的范围。
    let range = Range::new(self.last_line_visited, upto);

    // line iter: 开始和结束位置，line的分隔符
    let mut stepper = LineStep::new(
      self.config.line_term.as_byte(),
      range.start(),
      range.end(),
    ...
    // 在buf中查找line
    while let Some(line) = stepper.next_match(buf) {
      // 将当前行传递给sink_after_context方法进行处理。如果处理失败（即返回false），则返回Ok(false)
      if !self.sink_after_context(buf, &line)? {
        return Ok(false);
      }

      if self.after_context_left == 0 {
        // 检查self.after_context_left的值，如果为0，则表示已经找到了足够的上下文行数，退出循环。
        break;
      }
    }
    Ok(true)
  ...
  fn match_by_line_slow(
    &mut self,
    buf: &[u8],
  ) -> Result<bool, S::Error> {
    debug_assert!(!self
      .searcher
      .multi_line_with_matcher(&self.matcher));
  ...
  fn match_by_line_fast(
    ...
    while !buf[self.pos()..].is_empty() {
      if self.config.stop_on_nonmatch && self.has_matched {
        debug!("stop on non_match or has_matched");
  ...
  fn find_by_line_fast(
    ...
    debug!("find_by_line_fast arg: {}", String::from_utf8_lossy(buf));
  ...
  fn count_lines(&mut self, buf: &[u8], upto: usize) {
    if let Some(ref mut line_number) = self.line_number {
      ...
      let count =
        lines::count(slice, self.config.line_term.as_byte());
      *line_number += count;
