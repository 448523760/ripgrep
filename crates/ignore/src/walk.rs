...
impl DirEntryRaw {
  ...
  fn metadata_internal(&self) -> Result<fs::Metadata, Error> {
    ...
    .map_err(|err| {
      Error::Io(io::Error::from(err)).with_path(&self.path)
    })
  }
...
/// ---
///
/// * 首先，检查全局通配符（glob）覆盖。如果路径与全局通配符匹配，则匹配过程停止。
/// 然后，仅当匹配路径的全局通配符是一个忽略通配符时，路径才会被跳过。
///（覆盖通配符是一个白名单通配符，除非以!开头，否则它是一个忽略通配符。）
///
/// * 其次，检查忽略文件。忽略文件目前只包括来自于git忽略文件
/// （.gitignore、.git/info/exclude和配置的全局gitignore文件）、
/// 纯文本的.ignore文件（格式与gitignore文件相同）或显式添加的忽略文件。
/// 优先级顺序为：.ignore、.gitignore、.git/info/exclude、全局gitignore，
/// 最后是显式添加的忽略文件。请注意，不同类型的忽略文件之间的优先级不受目录层次结构的影响；
/// 任何.ignore文件都会覆盖所有.gitignore文件。在每个优先级级别内，更嵌套的忽略文件的优先级高于较少嵌套的忽略文件。
///
/// * 第三，如果前一步得到了一个忽略匹配，那么所有匹配都会停止，并跳过该路径。如果得到了一个白名单匹配，则匹配继续。
/// 后续的匹配器可以覆盖白名单匹配。
///
/// * 第四，除非路径是一个目录，否则会对文件类型进行匹配。
/// 与前面一样，如果得到了一个忽略匹配，那么所有匹配都会停止，并跳过该路径。
/// 如果得到了一个白名单匹配，则匹配继续。
///
/// * 第五，如果路径没有被列入白名单，并且它是隐藏的，则跳过该路径。
///
/// * 第六，除非路径是一个目录，否则将文件的大小与最大文件大小限制进行比较。如果超过限制，则跳过该文件。
///
/// * 第七，如果路径已经通过了前面的所有检查，则在迭代器中返回该路径。
///
///
#[derive(Clone)]
...
enum Sorter {
  ...
  ByPath(
    Arc<
      dyn Fn(&Path, &Path) -> cmp::Ordering + Send + Sync + 'static,
    >,
  ),
}
...
impl WalkBuilder {
  ...
  /// are still applied.
  pub fn add_ignore<P: AsRef<Path>>(
    &mut self,
    path: P,
  ) -> Option<Error> {
...
impl<'s> Worker<'s> {
  ...
  fn generate_work(
    ...
    let is_symlink =
      dent.file_type().map_or(false, |ft| ft.is_symlink());
...
fn check_symlink_loop(
  ...
  for ig in
    ig_parent.parents().take_while(|ig| !ig.is_absolute_parent())
  {
...
mod tests {
  ...
  fn walk_collect_entries_parallel(
    builder: &WalkBuilder,
  ) -> Vec<DirEntry> {
