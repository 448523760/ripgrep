...
impl Matcher for RegexMatcher {
  ...
  fn find_candidate_line(
    ...
    debug!("start to find candidate line for: {}", String::from_utf8_lossy(haystack));
...
impl Matcher for StandardMatcher {
  ...
  fn shortest_match_at(
    ...
    let result = self.regex.search_half(&input).map(|hm| hm.offset());

    if let Some(value) = result {
      debug!("result matched at: {}", value);
    } else {
      debug!("none value for the input")
    }
